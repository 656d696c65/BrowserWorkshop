...
export function ConversionView(props: {
    ...
    }, [numValue, fromUnit, toUnit, isValidValue, props.convert])
    ...
    return (
        ...
        >
            <InputText value={value} onChange={(v) => setValue(v ?? "")} type="number" placeholder="Enter a value" />
            ...
            {result !== null && (
                ...
                >
                    {value} {fromLabel} = <span className={css({ color: "primary" })}>{formatResult(result)}</span>{" "}
                    {toLabel}
                </div>
            )}
        </div>
