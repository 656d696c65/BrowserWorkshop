...
>(parameters: {
    ...
    const abortController = parameters.signal
        ? undefined
        : new AbortController()
    ...
    try {
        ...
        const response = await fetch(
            new URL(`${apiBaseUrl}${parameters.routeDefinition.path}`),
            {
                method: "POST",
                headers,
                credentials: "include",
        ...
        if (parameters.hasToastMessage) {
            ...
            try {
                ...
                if (parsed?.nested && typeof parsed.nested === "object") {
                    validationMessages = Object.entries(
                        parsed.nested as Record<string, string[]>,
                    )
                        .map(
                            ([field, errors]) =>
                                `${field}: ${errors.join(", ")}`,
